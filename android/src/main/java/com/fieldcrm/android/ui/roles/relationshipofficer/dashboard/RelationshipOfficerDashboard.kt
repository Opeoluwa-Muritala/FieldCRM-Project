...
@Composable fun RelationshipOfficerDashboard(metrics: DashboardMetrics?, loading: Boolean, error: String?, onOpen: (Screen)->Unit, onSignOut:()->Unit) {
    val m=metrics?.data?.metrics
    WorkspaceDashboardPage("Relationship Officer", "Prepare applications and follow up outstanding customer requirements.", listOf(
    ...
    ), listOf(WorkspaceAction("My Application Work", Screen.MyQueue), WorkspaceAction("Create Application", Screen.CreateApplication), WorkspaceAction("Find Customers and Applications", Screen.SearchResults)), loading,error,onOpen,onSignOut)
}
