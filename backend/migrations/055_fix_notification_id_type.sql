-- notifications.id is a text identifier in the existing schema. Recreate the
-- secure writer with the matching result type so PostgreSQL can materialize
-- its RETURNING row.
DROP FUNCTION IF EXISTS public.app_create_notification(uuid, uuid, uuid, text, text, text);
CREATE FUNCTION public.app_create_notification(
  target_user_id uuid, target_org_id uuid, target_application_id uuid,
  target_title text, target_message text, target_type text
)
RETURNS TABLE (
  id text, title text, message text, created_at timestamptz,
  is_read boolean, application_id uuid, type text
)
LANGUAGE plpgsql SECURITY DEFINER
SET search_path = pg_catalog, public, pg_temp
AS $$
BEGIN
  IF target_org_id IS NULL OR NOT EXISTS (SELECT 1 FROM public.organisations o WHERE o.id = target_org_id) THEN
    RAISE EXCEPTION 'invalid notification organisation';
  END IF;
  IF NOT EXISTS (SELECT 1 FROM public.users u WHERE u.id = target_user_id AND u.org_id = target_org_id AND u.deleted_at IS NULL) THEN
  ...
  IF target_application_id IS NOT NULL AND NOT EXISTS (
    ...
    WHERE la.id = target_application_id AND la.org_id = target_org_id AND la.deleted_at IS NULL
  ) THEN
